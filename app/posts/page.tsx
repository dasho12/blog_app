...
export default function PostsPage() {
  ...
  const tag = searchParams.get("tag") || null; // URL-ээс tag авна
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      if (status === "authenticated") {
        try {
          const url = tag
            ? `/api/posts?page=${pagination.currentPage}&limit=${
                pagination.limit
    ...
    fetchPosts();
  }, [status, pagination.currentPage, tag]); // tag нэмэгдсэн
    ...
    <div className="flex flex-col items-center justify-center py-16">
      <div className="text-center mb-8">
        <h1 className="text-4xl font-bold text-gray-900 flex items-center justify-center">
          ...
          Бүх Нийтлэлүүд {tag && <span className="text-blue-600">({tag})</span>}
        </h1>
        {session && (
